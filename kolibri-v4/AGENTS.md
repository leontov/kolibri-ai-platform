...
## Kolibri V4 visual direction

- The supplied mobile ChatGPT screenshots are the visual source of truth: quiet white chat canvas, large touch targets, bold system typography, rounded outlined controls, and sparse chrome.
- Chat is the home screen. Estimates, KS-2/KS-3 documents, and other vertical tools appear only after the user asks for them; they must not turn the empty chat screen into a dashboard.
- The main navigation is a left push/overlay drawer. Projects and settings are full mobile surfaces in the same visual language.
- Keep no more than two primary surfaces visible at once. Dense estimate editing opens as its own mobile workspace instead of being squeezed beside chat.
- Use Kolibri naming and product actions while preserving the reference hierarchy and spacing. Do not introduce enterprise-dashboard cards, decorative gradients, or 1C-like table density on the start screen.
